...
def _fetch_history(ticker: str, today: date | None = None) -> list[list]:
    ...
    source — the switch flips charts and picks together. Legacy behaviour is
    unchanged: a missing live quote leaves the chart ending yesterday; the
    Xignite path treats that as a ChartError (a stale quote means the name
    did not trade today)."""
    if today is None:
